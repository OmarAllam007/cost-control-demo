...
GROUP BY resource_id , resource_name', [$project->id]))->map(function ($resource) {
  ...
  c.resource_id,
  c.to_date_unit_price              AS to_date_unit_price,
  ...
  c.remaining_unit_price            AS remaining_unit_price,
  SUM(c.remaining_qty)                   AS remaining_qty,
  SUM(c.remaining_cost)                  AS remain_cost,
  c.completion_unit_price           AS completion_unit_price,
...
GROUP BY c.resource_id ,c.to_date_unit_price ,c.remaining_unit_price ,c.completion_unit_price', [$project->id, $chosen_period_id]))->map(function ($resource) {
