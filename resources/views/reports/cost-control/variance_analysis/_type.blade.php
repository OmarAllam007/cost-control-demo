...
@foreach ($typeData as $discipline => $disciplineData)
    <tr class="discipline {{slug($type)}} hidden">
        <td class="resource-cell right-border">
            <a href="#" data-target="{{slug($type)}}-{{slug($discipline)}}"><i class="fa fa-plus-circle"></i> {{$discipline}}</a>
        </td>
    ...
    @foreach($disciplineData as $resource)
        <tr class="{{slug($type)}}-{{slug($discipline)}} resource hidden">
            <td class="resource-cell right-border">
            ...
            <td class="number-cell">{{number_format($price_var = $resource->budget_unit_price - $resource->to_date_unit_price, 2)}}</td>
            <td class="number-cell right-border">{{number_format($price_var * $resource->to_date_qty, 2)}}</td>
            ...
            <td class="number-cell">{{number_format($qty_var = $resource->to_date_allowable_qty - $resource->to_date_qty, 2)}}</td>
            <td class="number-cell right-border">{{number_format($qty_var * $resource->budget_unit_price, 2)}}</td>

            <td class="number-cell">{{number_format($resource->cost_unit_price_var, 2)}}</td>
            <td class="number-cell right-border">{{number_format($resource->cost_qty_var, 2)}}</td>
        </tr>
    @endforeach
@endforeach