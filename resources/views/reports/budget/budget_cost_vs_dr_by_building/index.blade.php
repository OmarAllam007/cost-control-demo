...
@section('title', 'Budget Cost vs Dry Cost by Building')

@section('header')
    <div class="display-flex">
        <h4 class="flex">Budget Cost vs Dry Cost by Building &mdash; {{$project->name}}</h4>
...
@section('body')
    <table class="table table-condensed table-bordered" id="report-table">
        ...
        <tr class="bg-primary">
            ...
            <th class="col-sm-2">Dry Cost</th>
            <th class="col-sm-2">Difference</th>
        ...
        @foreach($tree as $wbs_level)
            @include('reports.budget.budget_cost_vs_dr_by_building._recursive', ['wbs_level' => $wbs_level, 'depth' => 0])
        @endforeach
        </tbody>
    </table>
@endsection
