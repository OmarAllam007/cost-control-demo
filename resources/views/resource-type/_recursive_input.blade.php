<li>
    <div class="tree--item">
        <label class="tree--item--label">
            <input type="radio" class="tree-radio" name="{{isset($input)? $input : 'parent_id'}}" value="{{$division->id}}" {{$value == $division->id? 'checked' : ''}}>
            <a href="#resource-type-children-{{$division->id}}" class="node-label" data-toggle="collapse">{{$division->label}}</a>
        </label>
    </div>
