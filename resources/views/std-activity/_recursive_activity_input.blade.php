        ...
        @if ($division->activities->count())
            <ul class="list-unstyled">
                @foreach($division->activities as $activity)
                    <li>
                        <label>
                            <input type="radio" class="tree-radio activity-input" name="{{$input or 'std_activity_id'}}" value="{{$activity->id}}" data-label="{{$activity->name}}" {{$value == $activity->id? 'checked="checked' : ''}} v-model="activity_id">
                            {{$activity->name}}
                        </label>
                    </li>
                @endforeach
